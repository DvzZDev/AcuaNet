...
const EmbalseGrafico = ({ porcentaje }) => {
  ...
  return (
    <div className="relative flex sm:h-full w-full items-center justify-center my-3 sm:my-0">
      <span className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 transform bg-gradient-to-r from-purple-500 to-blue-500 bg-clip-text text-xl font-black text-transparent sm:text-sm md:text-xl">
        {`${porcentaje.toFixed(2)} %`}
      </span>

      <div className="flex h-[15rem] w-[15rem] items-center justify-center sm:h-[7.8rem] sm:w-[7.8rem] md:h-[12rem] md:w-[12rem]">
        <Doughnut
          data={data}
          options={options}
        />
